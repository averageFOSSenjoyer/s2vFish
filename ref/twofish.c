// MODIFIED BASED ON SOURCE BY DREW CSILLAG
// compiled with gcc 12.2.0 and flags -O3 -fomit-frame-pointer -Wall -m32
...
extern inline void encrypt(u32 K[40], u32 S[4][256], BYTE PT[16]) {
    u32 R0, R1, R2, R3;
    u32 T0, T1;
...
extern inline void decrypt(u32 K[40], u32 S[4][256], BYTE PT[16]) {
    u32 T0, T1;
    u32 R0, R1, R2, R3;
...
int main() {
    ...
    FILE* fi = fopen("./testData.txt", "r");
    FILE* fo = fopen("./results.txt", "w");

    fgets(buf, sizeof(buf), fi);
    key[0] = strtoll(buf, &end, 16); 
    for (int i = 1; i < 16; i++)
        key[i] = strtoll(end, &end, 16);
    ...
    while (fgets(buf, sizeof(buf), fi)) {
        text[0] = strtoll(buf, &end, 16); 
        for (int i = 1; i < 16; i++)
            text[i] = strtoll(end, &end, 16);
...
// gcc -O3 -fomit-frame-pointer -Wall -m32 twofish.c -o twofish